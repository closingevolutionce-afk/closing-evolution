...
import { levels, moduleOrder, getLevelForModule } from '@/lib/knowledge'
...
// accessLevel 'aucun' : rien n'est débloqué, même pas Fondations.
// accessLevel 'apercu' : élève à accès limité (ex. acompte versé, solde en
// attente) — seul le niveau Fondations reste débloquable, le reste du
// parcours attend le passage en 'complet' par le coach.
export function isModuleUnlocked(moduleId, completed, accessLevel = 'complet') {
  if (accessLevel === 'aucun') return false
  if (accessLevel === 'apercu' && getLevelForModule(moduleId)?.id !== 'fondations') {
    return false
  }
